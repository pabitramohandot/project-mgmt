    ...
    <div className="animate-fade-in">
      ...
      <div className="page-header" style={{ marginBottom: '2rem' }}>
        <div>
          ...
          <p className="page-subtitle">Customize branding, URLs, active status, and view associated staff accounts.</p>
        </div>
      </div>
          ...
          <form onSubmit={handleFormSubmit}>
            ...
            <div className="form-group" style={{ marginTop: '1rem' }}>
              <label style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                ...
                />
                <span style={{ fontSize: '0.9rem', fontWeight: 500 }}>Active Company Status</span>
              </label>
            </div>
            ...
            <button
              ...
              style={{ width: '100%', marginTop: '1rem', height: '42px', display: 'flex', gap: '0.5rem', justifyContent: 'center' }}
              disabled={submitting}
            >
              <Save size={18} />
              <span>{submitting ? 'Saving changes...' : 'Save Company Details'}</span>
            </button>
          </form>
        </div>
        ...
        <div className="card" style={{ padding: '2rem' }}>
          ...
          {users.length === 0 ? (
            <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-secondary)' }}>
              No user accounts found for this company. Go to the Users panel to create a user and assign them here.
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {users.map((u) => (
                <div key={u._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-color)', borderRadius: '12px' }}>
                  <div>
                    <h4 style={{ fontWeight: 600 }}>{u.username}</h4>
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Created {new Date(u.createdAt).toLocaleDateString()}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <ShieldCheck size={14} style={{ color: u.role === 'company_admin' ? 'var(--accent-primary)' : 'var(--text-muted)' }} />
                    <span style={{ fontSize: '0.8rem', textTransform: 'capitalize', fontWeight: 500 }}>{u.role.replace('_', ' ')}</span>
                  </div>
                </div>
              ))}
            </div>
